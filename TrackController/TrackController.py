...
#create run button and maybe stop button
...
class TrackController(QMainWindow):
    ...
    def __init__(self, tc_ID = 0, auto_Run = False):
        ...
        #array of executable python bytecode converted from plc logic
        self.executable = []
    ...
    def run_Logic(self):
        ...
        if self.filename != "":
            ...
            comp = PLCCompiler.PLCCompiler()
            if comp.compile_PLC(logic_File) == True:
                self.run_PLC = True
            else:
                self.run_PLC = False
        ...
        while self.run_Vital == True:
            ...
            if self.run_PLC == True:
                ...
                try:
                    while pc < p_Length:
                        #exec is generally unsafe but it should be fine
                        #since the exectuable text is generated by the track controller
                        exec(self.executable[pc], locals())
                        pc += 1

                        #checks time to prevent infinite loops and kill logic that is too slow
                        time_Diff = time.perf_counter() - start_Time
                        assert time_Diff < self.target_Time, "PLC execution too slow."
            ...
            print(str(self.next_Track_State["red_A_1"].switches[0]))
            #self.next_Track_State["red_A_1"].switches[0] = True
